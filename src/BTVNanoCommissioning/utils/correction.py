    ...
    for SF in config[campaign].keys():
        ...
        ## rochester muon momentum correction
            ...
            if os.path.exists(
                ...
                correction_map["JMAR_cfg"] = {j: f for j, f in config[campaign]["JMAR"].items()}
...
def btagSFs(jet, correct_map, weights, SFtype, syst=False):
    ...
    for i, sys in enumerate(systlist):
        ...
        for nj in range(ak.num(alljet.pt)[0]):
            ...
            jet.btagDeepFlavB = ak.fill_none(jet.btagDeepFlavB, 0.0)
...
#            sfs = 1.0
            ...
            mu_eta = np.clip(mu_eta, 0.0, 2.4)
            mask = mu_pt > 40
            sfs = 1.0
            if "correctionlib" in str(type(correct_map["MUO"])):
                if "ID" in sf or "Reco" in sf:
                    #mu_pt = np.where(mu_pt < 15, 15, mu_pt)
#                    mu_pt_low = np.where(mu_pt >= 15, 14.9, mu_pt)
#
#                    sfs_low = np.where(
#                        ~mask & ~masknone,
